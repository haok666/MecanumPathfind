...
def visualize_path(pathCoords, obstacleList, filename="path.png"):
    ...
    for x, y, side in obstacleList:
        plt.gca().add_patch(
            plt.Rectangle((x - side / 2, y - side / 2), side, side,
                          color="red", alpha=0.4)
        )
    ...
    if pathCoords:
        ...
        plt.plot(xs, ys, "b-", lw=1.4, label="centre path")

        halfLen = robotLength / 2
        halfWid = robotWidth / 2
        step = max(1, len(pathCoords) // 25)
        ...
        for idx in range(0, len(pathCoords), step):
            ...
            pts = (rot @ rect.T).T + np.array([cx, cy])
            plt.plot(pts[:, 0], pts[:, 1], "g-", lw=0.8)

    plt.axis("equal")
    plt.grid(True)
    plt.xlabel("x (m)")
    plt.ylabel("y (m)")
    plt.title("FTC Robot Path")
    plt.legend()
    saveDir = "C:/Users/Kenny/Desktop/Coding/FTC"
    os.makedirs(saveDir, exist_ok=True)
    plt.savefig(os.path.join(saveDir, filename), dpi=300, bbox_inches="tight")
    plt.close()
