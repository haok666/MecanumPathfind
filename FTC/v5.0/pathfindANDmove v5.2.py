...
Single-file program. Outputs visualization PNG and wheel rotation CSV (wheel power in percent).
...
outputCsvPath = 'wheel_power_percent.csv'  # logs wheel power in percent

# control sampling
dt = 0.1

# optional image map (black=obstacle). Set to None to use obstacles list below.
# If this path is not found the script will fall back to the obstacles list.
imagePath = '/mnt/data/grid_debug.png'  # change or set to None
...
# occupancy grid builder (graceful fallback if image missing)
def build_obstacle_map():
    ...
    if imagePath:
        if os.path.exists(imagePath):
            ...
            # try common alternatives (remove leading slashes or normalize)
            alt = imagePath.lstrip('/\\')
            alt2 = os.path.normpath(imagePath)
            if os.path.exists(alt):
                ...
                print('Warning: imagePath not found:', imagePath, 'Falling back to obstacle list.')
                useImage = False

    if useImage and imgPath is not None:
        img = Image.open(imgPath).convert('L')
        img = img.resize((gridSizeX, gridSizeY), Image.NEAREST)
        arr = np.array(img)
        # treat dark pixels as obstacles
        grid |= (arr < 128)
    else:
        ...
        for (ox, oy, ow, oh) in obstacles:
            ...
            # ensure slice bounds correct and ordered
            lx = min(gx0, gx1)
            hx = max(gx0, gx1)
            ly = min(gy0, gy1)
            hy = max(gy0, gy1)
            grid[lx:hhx+1, ly:hy+1] = True
